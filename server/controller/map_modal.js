...
export async function map_modal(req, res, next) {
    const place = req.body;
    console.log(place)
    try {
        fetch(`http://openapi.seoul.go.kr:8088/${encodeURIComponent(key)}/xml/citydata/1/5/${encodeURIComponent(place)}`)
            ...
            .then(xmlData => {
                ...
                res.status(201).json({
                    placeName, AREA_CONGEST_LVL,
                    AREA_CONGEST_MSG, peopleNum, temp, MAX_TEMP,
                    MIN_TEMP, HUMIDITY, WIND_SPD, rain
                });
            })
    } catch (e) {
        console.log('요청하신 장소가 없습니다.')
        next();
    }
}
